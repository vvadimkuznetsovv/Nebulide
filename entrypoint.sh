...
# Claude Code hooks + statusLine теперь регистрирует САМ бэкенд (Go) на старте —
# кросс-платформенно (Linux в Docker и локально на Windows), один механизм для обеих ОС.
# См. backend/services/claudehooks.go (RegisterClaudeHooks) → ~/.claude/settings.json.
mkdir -p /root/.claude
