...
// JSON с живым контекстом (context_window, model, cost). Мы POSTим его на бэкенд (→ чат
// показывает контекст/токены) И печатаем компактную строку для самого терминала.
//
// Env: NEBULIDE_HOOK_TOKEN / NEBULIDE_HOOK_URL / NEBULIDE_INSTANCE_ID (как у хука).
...
process.stdin.on('end', async () => {
  let inp = {};
  try { inp = JSON.parse(raw); } catch { /* ignore */ }

  // Форвард контекста на бэкенд (как event=StatusLine).
  ...
  // Компактная строка статуса для терминала: «Sonnet 4.6 · ctx 13% (26k / 200k)».
